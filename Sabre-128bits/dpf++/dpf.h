...
#define _mm256_setlo_m128i(v1)  _mm256_insertf128_si256(_mm256_castsi128_si256(v1), _mm_setzero_si128(), 1)

 
static inline void traverse2(const lowmc::lowmc<256,14,63> & prgkey, const __m128i & seed,
    const uint8_t cw_t[2], const __m128i & cw, const uint8_t prev_t,
    __m128i s[2], uint8_t t[2], int lsbmask = 0b11)
{
    __m256i * S = reinterpret_cast<__m256i*>(s);
    *S = _mm256_setlo_m128i(clear_lsb(seed, 0b11));
   
     //__m256i E_S = prgkey.encrypt(S);
    // S = S ^ E_S; //prgkey.encrypt(S);
...
static inline void traverse(const lowmc::lowmc<256,14,63> & prgkey, const __m128i & seed, const bool direction,
    const uint8_t cw_t, const __m128i & cw, const uint8_t prev_t,
    __m128i & s, uint8_t & t, int lsbmask = 0b00)
{
    auto S = _mm256_setlo_m128i(clear_lsb(seed, 0b11));
    __m256i E_S = prgkey.encrypt(S);
    S = S ^ E_S; //prgkey.encrypt(S);

    __m128i * s_ = reinterpret_cast<__m128i*>(&S);

    t = get_lsb(s_[direction]) ^ (cw_t & prev_t);
    s = clear_lsb(xor_if(s_[direction], cw, !prev_t), lsbmask);
} // dpf::traverse
