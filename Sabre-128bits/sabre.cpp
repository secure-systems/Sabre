...
int main(int argc, char * argv[])
{
	...
	constexpr size_t interval_len = nitems - 1;

	uint8_t * t0 = (uint8_t *) malloc ( sizeof(uint8_t) * nitems);
	leaf_t * output0 = (leaf_t *) std::aligned_alloc(sizeof(__m256i), nitems * sizeof(leaf_t));

	uint8_t * t1 = (uint8_t *) malloc ( sizeof(uint8_t) * nitems);
	leaf_t * output1 = (leaf_t *) std::aligned_alloc(sizeof(__m256i), nitems * sizeof(leaf_t));

	__evalinterval(dpfkey0, 0, interval_len, output0, t0);
	__evalinterval(dpfkey1, 0, interval_len, output1, t1);

	for(size_t j = 0; j < nitems; ++j)
	{
	 if(t0[j] != t1[j]) printf("j =  %zu\n",  j);
	} 
	...
	for(size_t j = 0; j < nitems; ++j)
	{
		...
		for(size_t k = 0; k < BLOCK_LEN; ++k)
		 {
		 	if(flags0[j].bits[k] != flags1[j].bits[k]) equal = false;
		 }

		 if(!equal) printf("bitsliced index = %zu \n\n", j);
	}
