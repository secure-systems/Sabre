...
int main(int argc, char * argv[])
{
  ...
  {
    arc4random_buf(&virtualaddress[j], sizeof(__m128i));
  }
...
size_t len0 =  ((2 * sizeof(blockT)) + (rounds * 2 * depth * sizeof(blindT)) + (2 * 2 *  blocklen * depth * sizeof(node_t)) +  (2 * 2 * depth * sizeof(node_t)))/sizeof(blockT);
size_t len1 =  ((2 * sizeof(blockT)) + (rounds * 2 * depth * sizeof(blindT)) + (2 * 2 *  blocklen * depth * sizeof(node_t)) +  (2 * 2 * depth * sizeof(node_t)))/sizeof(blockT);
size_t len2 =   ((rounds * 2 * depth * sizeof(blindT)) + (2 *  blocklen * depth * sizeof(node_t)) +  (2 * 2 * depth * sizeof(node_t)))/sizeof(blockT);
...
//         uint8_t * _t0 = (uint8_t *) malloc ( sizeof(uint8_t) * (nitems + 1));
//         leaf_t * _output0 = (leaf_t *) std::aligned_alloc(sizeof(__m256i), (nitems + 1) * sizeof(leaf_t));
//         dpfkey0.evalinterval(0, (nitems - 1), _output0, _t0);
//         __m128i share0 = _mm_setzero_si128();
//         for(size_t j = 0; j < nitems; ++j)
//         {
//            if(_t0[j]) share0 = share0 ^ virtualaddress[j];
//         }


//         uint8_t * _t1 = (uint8_t *) malloc ( sizeof(uint8_t) * (nitems + 1));
//         leaf_t * _output1 = (leaf_t *) std::aligned_alloc(sizeof(__m256i), (nitems + 1) * sizeof(leaf_t));
//         dpfkey1.evalinterval(0, (nitems - 1), _output1, _t1);

  
// auto start_evalfull = std::chrono::high_resolution_clock::now();
//           __m128i share1 = _mm_setzero_si128();
//         for(size_t j = 0; j < nitems; ++j)
//         {
//            if(_t1[j]) share1 = share1 ^ virtualaddress[j];
//         }

//         __m128i share = share0 ^ share1;
//          std::cout << "share: " << share[0] << std::endl;
//          std::cout << "share: " << virtualaddress[target][0] << std::endl; 
...
//   std::cout << "elapsed_evalfull = " << elapsed_evalfull.count() << std::endl;
 ...
 Simulator sim(depth, randomness_prgkey, prgkey, seed0_, seed1_, seed2_, len);  
  ...
  std::cout << "elapsed_sim = " << elapsed_sim.count() << std::endl;// << " ---> " << elapsed_sim.count()/16 << std::endl;
  ...
  std::cout << "elapsed_ver = " << elapsed_ver.count() << " ---> " << elapsed_ver.count()/16 << std::endl;
