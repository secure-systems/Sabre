...
{   
  ...
  constexpr size_t nrows_p2_root   = 3 * blocklen;
   constexpr size_t nrows_p2_middle = blocklen + ( ( (2 * rounds * 3 * numofboxes)  + 2 + (2 * blocklen)) + (128 - (( (2 * rounds * 3 * numofboxes)  + 2 + (2 * blocklen)) % 128))) * (depth-1);
  constexpr size_t nrows_p2_leaf = blocklen;  
  constexpr size_t nrows_p2 = nrows_p2_root;//  nrows_p2_root + nrows_p2_middle + nrows_p2_leaf;
  ...
  constexpr size_t nrows_m =   (((4 * (rounds + 1) * 3 * numofboxes) + (4 * blocklen) + 4) + (128 -  (((4 * (rounds + 1) * 3 * numofboxes) + (4 * blocklen) + 4) % 128)) ) * (depth-1);
  constexpr size_t nrows_l =   blocklen;
  constexpr size_t nrows_total = nrows_r;//  nrows_r + nrows_m + nrows_l;
  ...
  const uint8_t * P2_P0_view_transpose = reinterpret_cast<const uint8_t* >((&P2_P0_view->root));
  const uint8_t * P2_P1_view_transpose = reinterpret_cast<const uint8_t* >((&P2_P1_view->root));

  const uint8_t * P1_view_transpose_t  = reinterpret_cast<const uint8_t* >((&P1_view_->root));
  const uint8_t * P0_view_transpose_t  = reinterpret_cast<const uint8_t* >((&P0_view_->root));
...
{       
  ...
  const uint8_t * P2_P0_view_transpose = reinterpret_cast<const uint8_t* >((&P2_P0_view_V->root));
  const uint8_t * P2_P1_view_transpose = reinterpret_cast<const uint8_t* >((&P2_P1_view_V->root));

  const uint8_t * P1_view_transpose_t  = reinterpret_cast<const uint8_t* >((&gen_view1_64->root));
  const uint8_t * P0_view_transpose_t  = reinterpret_cast<const uint8_t* >((&gen_view0_64->root));
